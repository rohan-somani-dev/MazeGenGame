...
public class Grid implements Updater {
	...
	public void resetMaze(Node playerPos) {
		for (Node n : getNodes()) {
			n.resetWalls();
		...
		playerPos.setOverlayState(CellState.PLAYER);
	}
	...
	void mazeGenFinished() {
		...
		System.out.printf("(%s, %s)\n", topChoice.indexX, topChoice.indexY);
		int maxDistance = 0;
	...
	public void clearPathStates() {
		for (Node n : getNodes()) {
			...
			n.onPath = false;
			n.parent = null; 
			if (n.getBaseState() == CellState.PATH) {
				n.setBaseState(CellState.BACKGROUND);
