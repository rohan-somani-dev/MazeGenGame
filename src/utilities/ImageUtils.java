...
public class ImageUtils {
  ...
  public static BufferedImage tint(BufferedImage image, Color tintColor) {
    ...
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
         ...
         * rgb return a 32 bit int AAAAAAAA RRRRRRRR GGGGGGGG BBBBBBBB
         * the first bit shift (>>24) moves the first byte to the least
         * significant position, allowing us to mask it with 0xFF
         * removing any bytes preceding it, isolating the channel which
         * is in this case alpha. repeat for all other shifts with 16,
         * 8, and finally 0, because the blue channel is already in the
         * least significant bit so no shifting is needed. note that we
         * still mask, however, because otherwise int b would be the
         * whole 32 bits which is not what we want.
         */
        ...
        /*
         * finally, after doing the recalculations we can shift them
         * back int to the new 32 bit int the same way we got them out.
         * notice we're using bitwise | (or) instead of & (and) so that
         * the channels get added rather than masking each other.
         */

        int out = getColorInt(a, newR, newG, newB);
        tinted.setRGB(x, y, out);

        // tldr: fancy bit shifting, image multiplication, leave us with
        // a tinted image.
        // also, i had to google this, but i hope this explanation was
        // an indication of my understanding of the code.
      }
    }
  ...
  public static BufferedImage resizeImage(BufferedImage image, int targetWidth, int targetHeight) {
    ...
    while (width / 2 >= targetWidth && height / 2 >= targetHeight) {
      ...
      Graphics2D g = Setup.prepareGraphics(temp.createGraphics());
      g.drawImage(curr, 0, 0, width, height, null);
      g.dispose();
      curr = temp;
    }

    BufferedImage out = new BufferedImage(targetWidth, targetHeight, BufferedImage.TYPE_INT_ARGB);
    Graphics2D g = Setup.prepareGraphics(out.createGraphics());
