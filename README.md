...
Download the `src/` and `resources/` folders. Place them in the same directory. We'll call that `root/`. From the root,
...
* Swing is generally very ugly, so most of my time on this project was spent making it... not ugly. The UI package
  itself is the biggest package, containing overrides for buttons and drop-downs currently, but more will be added as
  development ensues.
